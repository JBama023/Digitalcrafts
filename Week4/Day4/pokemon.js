...
const data = [
  ...
  async function fetchPokemonData() {
    ...
    for (const pokemon of data) {
        const addingCards = document.querySelectorAll(".cards")
            for (let index = 0; index < addingCards.length; index ++) {
                const item = addingCards[index];
                const cardImg = document.createElement("img");
                cardImg.src = pokemon[index].sprites.front;
                item.append(cardImg)
            }
    }
